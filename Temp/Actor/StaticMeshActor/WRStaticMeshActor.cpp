...
AWRStaticMeshActor::AWRStaticMeshActor(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	this->SetCanBeDamaged(false);

// 	StaticMeshComponent->SetCollisionProfileName(UCollisionProfile::BlockAll_ProfileName);
	this->GetStaticMeshComponent()->SetMobility(EComponentMobility::Movable);
...
// 	StaticMeshComponent->bUseDefaultCollision = true;
	...
	this->GetStaticMeshComponent()->AttachToComponent(RootComponent, FAttachmentTransformRules::KeepRelativeTransform);
}

void AWRStaticMeshActor::SetStaticMesh(UStaticMesh* InMesh)
{
	if (InMesh == nullptr)
		return;
	
	UStaticMeshComponent* _StaticMeshComponent = this->GetStaticMeshComponent();
	if (_StaticMeshComponent == nullptr)
		return;

	_StaticMeshComponent->SetStaticMesh(InMesh);
}

void AWRStaticMeshActor::SetMaterialParameterValue_Internal(FName InParameterName, float InValue, class UMeshComponent* InMeshComponent /*= nullptr*/)
{
	WRACTOR_SETMATERIAL_PARAMETER_VALUE(InParameterName, float, InValue, SetScalarParameterValue, (InMeshComponent == nullptr) ? GetStaticMeshComponent() : InMeshComponent);
}

void AWRStaticMeshActor::SetMaterialParameterValue_Internal(FName InParameterName, FLinearColor InValue, class UMeshComponent* InMeshComponent /*= nullptr*/)
{
	WRACTOR_SETMATERIAL_PARAMETER_VALUE(InParameterName, FLinearColor, InValue, SetVectorParameterValue, (InMeshComponent == nullptr) ? GetStaticMeshComponent() : InMeshComponent);
}
...
void AWRStaticMeshProjectile::TickDistance2Spawner(float DeltaSeconds)
{
	...
	if (fLength < 400.0f)
	{
		...
		case EWRFSM::Idle:
		{
			...
			EWRItemMainType Type = EWRItemMainType::SubWeapon;
