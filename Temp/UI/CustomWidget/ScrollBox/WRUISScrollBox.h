...
class WRSScrollPanel;

struct FWRScrollBoxInfo	//가상스크롤박스 필요정보
{
	int32 TotalNumContents;
	int32 NumScrolling[2];
	int32 StartSlotIdx = 0;	//시작 인덱스 
	float ScrollSlotLength;
};


/** SScrollBox can scroll through an arbitrary number of widgets. */
class WRUISScrollBox : public SCompoundWidget
{
public:
...
public:
	...
	SLATE_BEGIN_ARGS(WRUISScrollBox)
	...
	*/
	...
	float GetVrPhysicalScrollOffset();
	...
	*/
	...
	float GetCachedGeometryScale();
	FORCEINLINE int32 GetScrollDirection() const
	{ 
		return Orientation == Orient_Vertical ? 1 : 0; 
	}
...
public:
	...
	TSharedPtr<class WRSScrollPanel> ScrollPanel;
private:
